...
get_firefox_profile() {
    ...
    PROFILE_PATH=$(find ~/Library/Application\ Support/Firefox/Profiles/*.default-release -type d 2>/dev/null | head -n 1)
...
install_extensions() {
    ...
    for EXT in "${EXTENSIONS[@]}"; do
        ...
        open -a "Firefox" /tmp/addon-$EXT-latest.xpi &
        sleep 5
    done
...
# Menu to guide user
display_menu() {
    echo -e "${GREEN}macOS Firefox Privacy & Security Setup Script${NC}"
